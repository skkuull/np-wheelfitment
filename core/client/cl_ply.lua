...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            ...
            RPC.execute("np-wheelfitment_sv:saveWheelfitment", vin, currentFitmentsToSet)
        ...
        RPC.execute("np-wheelfitment_sv:setIsWheelFitmentInUse", false)
    end
end
...
Citizen.CreateThread(function()
    isWheelFitmentInUse = RPC.execute("np-wheelfitment_sv:getIsWheelFitmentInUse")
    isPlyWhitelisted = RPC.execute("np-wheelfitment_sv:checkIfWhitelisted")

    exports["np-polyzone"]:AddBoxZone("np-wheelfitment:zone1", vector3(480.66, -1317.94, 29.01), 4.8, 6.2, {heading = 28, minZ = 28.01, maxZ = 32.01, data = {id = "wheel_fitment_zone"}})
end)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    if zone ~= "np-wheelfitment:zone1" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            RPC.execute("np-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
AddEventHandler("np-polyzone:exit", function(zone)
    if zone ~= "np-wheelfitment:zone1" then
        return
    end
    inZone = false
end)
